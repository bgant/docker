...
# -d 0 serial number (rtl_eeprom) is older small RTL-SDR R820T (best results using classic settings)
# -d 1 serial number (rtl_eeprom) is newer large RTL-SDR Blog V4 R828D (best results using new settings)
rtl_433 -d 0 -f 915M -Y classic -s 250K -F json
